      ...
      <!-- <b-row class="mb-3">
        <b-col cols="12">
          <b-form-group
            ...
            title="Обратитесь в тех. поддержку для изменения этих данных"
            class="mb-3"
...
export default {
  ...
  created() {
    this.lastname = this.$store.state?.me?.lastname || "";
    this.firstname = this.$store.state?.me?.firstname || "";
    this.patronymic = this.$store.state?.me?.patronymic || "";
    this.birth_date = this.$store.state?.me?.birth_date || "";
    this.sex =
      this.sexList.filter(
        (item) => item.value == this.$store.state?.me?.sex
      )[0] || "";

    this.birth_place = this.$store.state?.me?.birth_place || "";
    this.email = this.$store.state?.me?.email || "";
    this.passport =
      `${this.$store.state?.me?.passport_series} ${this.$store.state?.me?.passport_number}` ||
      "";
    this.passport_issuer = this.$store.state.me.passport_issuer || "";
    this.passport_issue_date = this.$store.state.me.passport_issue_date || "";
    this.passport_code = this.$store.state?.me?.passport_code || "";
    this.inn = this.$store.state?.me?.inn || "";
    this.snils = this.$store.state?.me?.snils || "";
    this.address_city = this.$store.state?.me?.address_city || "";
    this.address_street = this.$store.state?.me?.address_street || "";
    this.address_house = this.$store.state?.me?.address_house || "";
    this.address_building = this.$store.state?.me?.address_building || "";
    this.address_flat = this.$store.state?.me?.address_flat || "";
  },
  mounted() {
    HTTP.get("regions").then((res) => {
      ...
      this.address_region =
        this.regions.filter(
          (item) => item.id == this.$store.state?.me?.address_region
        )[0] || "";
    });
  },
  methods: {
    sendForm(form) {
      ...
      this.isLoading = true;
      HTTPV2.patch("contractor/personal_data", dataSend)
        .then((res) => {
